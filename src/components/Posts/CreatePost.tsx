...
// Simple RoleTag component for this file
const RoleTag = ({ role, showTooltip = false }: { role: any; showTooltip?: boolean }) => (
...
export const CreatePost: React.FC<CreatePostProps> = ({ onClose, onSuccess }) => {
  ...
  const handleRoleRemove = (roleId: string) => {
    setSelectedRoles(selectedRoles.filter((r: any) => r.id !== roleId));
  };

  const handleRoleSelect = (role: any) => {
    if (!selectedRoles.find((r: any) => r.id === role.id)) {
